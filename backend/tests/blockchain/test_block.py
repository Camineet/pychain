...
def test_mined_block_difficulty_limits_to_1():
    last_block = Block(
        time.time_ns(),'test_last_hash', 'test_hash', 'test_data', difficulty=1, nonce=0
    )
    time.sleep(MINE_RATE / SECONDS)
    mined_block = Block.mine_block(last_block, 'bar')
...
def test_is_valid_block_bad_last_hash(last_block, block):
    ...
    with pytest.raises(Exception, match='The block last_hash must be equal to last_block.hash'):
        Block.is_valid_block(last_block, block) 
    
def test_is_valid_block_bad_proof_of_work(last_block, block):
    block.hash = 'fff'
    with pytest.raises(Exception, match='The proof of work requirement was not met. The block"s hash does not have leading zeros equal to the difficulty'):
        Block.is_valid_block(last_block, block)
