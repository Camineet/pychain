...
class Block:
    ...
    def mine_block(last_block, data):
        ...
        nonce = 0
        hash = crypto_hash(timestamp, data, last_hash, difficulty, nonce)
    ...
    def is_valid_block(last_block, block):
        ...
        if block.last_hash != last_block.hash:
            raise Exception('The block last_hash must be equal to last_block.hash')
        if hex_to_binary(block.hash)[0:block.difficulty] != '0' * block.difficulty:
            raise Exception('The proof of work requirement was not met. The block"s hash does not have leading zeros equal to the difficulty')
